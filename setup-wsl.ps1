...
Remove-Item -Path "c:/temp/tmatwood-ubuntu-24.04" -Recurse -Force -ErrorAction SilentlyContinue
...
podman rm -f tmatwood-ubuntu-24.04 2>$null
...
# Backup wsl.conf
wsl -d tmatwood-ubuntu-24.04 --cd / sudo cp /etc/wsl.conf /etc/wsl.conf.bak

# Restart systemd services (use -d to specify distro name)
wsl -d tmatwood-ubuntu-24.04 --cd / sudo systemctl daemon-reexec
wsl -d tmatwood-ubuntu-24.04 --cd / sudo systemctl restart systemd-resolved.service
wsl -d tmatwood-ubuntu-24.04 --cd / sudo systemctl unmask systemd-binfmt.service
wsl -d tmatwood-ubuntu-24.04 --cd / sudo systemctl restart systemd-binfmt.service
wsl -d tmatwood-ubuntu-24.04 --cd / sudo systemctl mask systemd-binfmt.service
